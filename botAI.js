...
const BotAI = {
  decide(ship, allShips){
    ...
    // приоритет цели: близость + класс + фокус союзников
    let best=null, bestScore=-Infinity;
    for(const e of targetPool){
      const d = dist(ship.pos, e.pos);
      let score = 200 - d*3;
      ...
      score += alliesTargeting*15; // фокус-огонь
    ...
    // урон под угрозой в этот ход, если остаться на месте и стрелять (для статиста)
    let predictedDanger = 0;
    if(weights.predictive){
    ...
    const shouldDodgeInstead = weights.predictive && dangerRatio > riskTolerance && Math.random() > weights.noise;
