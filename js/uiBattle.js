const UIBattle = {
  ...
  // Элементы создаются через createElementNS (не innerHTML) — innerHTML на SVG ненадёжен в части браузеров.
  ...
  renderRadar(engine){
    ...
    [10,20,30].forEach(km=>{
      svg.appendChild(this.svgEl('circle', {class:'radar-ring', cx, cy, r:km*scale}));
    });
    svg.appendChild(this.svgEl('circle', {class:'radar-boundary', cx, cy, r:30*scale}));

    [['A',POINT_A],['B',POINT_B]].forEach(([label,pos])=>{
      const p = toSvg(pos);
      svg.appendChild(this.svgEl('circle', {class:'radar-point', cx:p.x, cy:p.y, r:2*scale}));
      const t = this.svgEl('text', {class:'radar-point-label', x:p.x+2*scale+2, y:p.y+3});
      t.textContent = label;
      svg.appendChild(t);
    });

    const player = engine.player;
    const shapeEl = (cls, x, y) => {
      switch(cls){
        case 'destroyer': return this.svgEl('polygon', {points:`${x},${y-4} ${x+3.2},${y+3} ${x-3.2},${y+3}`});
        case 'cruiser': return this.svgEl('ellipse', {cx:x, cy:y, rx:3.6, ry:2.2});
        case 'battleship': return this.svgEl('rect', {x:x-4.2, y:y-2.6, width:8.4, height:5.2, rx:0.8});
        case 'submarine': return this.svgEl('polygon', {points:`${x},${y-3.6} ${x+3.6},${y} ${x},${y+3.6} ${x-3.6},${y}`});
        default: return this.svgEl('circle', {cx:x, cy:y, r:3});
      }
    };
    ...
    engine.allShips.forEach(s=>{
      ...
      const classes = ['radar-ship-'+(s.team===player.team?'ally':'enemy')];
      if(!s.alive) classes.push('radar-ship-dead');
      if(s.def.class==='submarine' && s.submerged) classes.push('radar-ship-submerged');
      if(s.isPlayer) classes.push('radar-ship-player');
      g.setAttribute('class', classes.join(' '));
      g.appendChild(shapeEl(s.def.class, p.x, p.y));
      svg.appendChild(g);
    });
  },
