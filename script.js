...
let bmActiveTab = 'weapons'; // 'weapons' | 'gear' | 'modules'
...
function renderTraders() {
  ...
  el.innerHTML = `
    ...
    <div class="bm-tabs">
      <button class="bm-tab ${bmActiveTab === 'weapons'  ? 'bm-tab-active' : ''}" onclick="switchBMTab('weapons')">🔫 Оружие</button>
      <button class="bm-tab ${bmActiveTab === 'gear'     ? 'bm-tab-active' : ''}" onclick="switchBMTab('gear')">🪖 Экипировка</button>
      <button class="bm-tab ${bmActiveTab === 'modules'  ? 'bm-tab-active' : ''}" onclick="switchBMTab('modules')">🔩 Модули</button>
      <button class="bm-tab ${bmActiveTab === 'medkits'  ? 'bm-tab-active' : ''}" onclick="switchBMTab('medkits')">💊 Медицина</button>
    </div>

    <div class="bm-tab-content" id="bm-tab-content">
      ${renderBMTabContent(bm.listings)}
    </div>
  `;
}
...
function switchBMTab(tab) {
  ...
  const bm = gameData.blackMarket;
  // Update tab buttons
  document.querySelectorAll('.bm-tab').forEach(btn => btn.classList.remove('bm-tab-active'));
  event.target.classList.add('bm-tab-active');
  document.getElementById('bm-tab-content').innerHTML = renderBMTabContent(bm.listings);
}

function renderBMTabContent(listings) {
  if (bmActiveTab === 'medkits') {
    return renderBMMedkits();
  }
  if (bmActiveTab === 'weapons') {
    return renderBMSectionNew('weapon',   '🔫 Оружие',   listings)
         + renderBMSectionNew('ammo',     '🟤 Патроны',  listings);
  }
  if (bmActiveTab === 'gear') {
    return renderBMSectionNew('armor',    '🪖 Броня',    listings)
         + renderBMSectionNew('gear',     '🎒 Снаряга',  listings);
  }
  if (bmActiveTab === 'modules') {
    return renderBMSectionNew('module',   '🔩 Модули',   listings);
  }
  return '';
}

function renderBMMedkits() {
  const rows = Object.values(MEDKITS).map(med => {
    if (med._craft) {
      return `
        <div class="bm-listing bm-cant-afford">
          ...
          <div class="bm-listing-info">
            ...
            <div class="bm-listing-meta"><span class="bm-desc">${escHtml(med.desc)}</span></div>
          </div>
      ...
      <div class="bm-listing ${canAfford ? '' : 'bm-cant-afford'}">
        ...
        <div class="bm-listing-right">
          ...
          <button class="btn-bm-buy ${canAfford ? '' : 'disabled'}"
                  ...
                  ${canAfford ? '' : 'disabled'}>Купить</button>
        </div>
      </div>`;
  }).join('');
  return `<div class="bm-section"><div class="bm-listings">${rows}</div></div>`;
}

function renderBMSectionNew(category, title, listings) {
...
function renderBMSection(category, title, listings) {
  return renderBMSectionNew(category, title, listings);
}
...
// ============================================================
// ГЕНЕРАЦИЯ РЫНКА — 3 лота на каждую позицию, все типы предметов
// ============================================================

const LOTS_PER_ITEM = 3; // количество лотов на каждую уникальную позицию

function generateBlackMarket() {
  const listings = [];
  const activeTiers = [1, 2, 3, 4].filter(t => TIERS[t] && TIERS[t].isActive);

  // Вспомогательная функция: добавить N лотов для конкретного предмета
  function addLots(category, item, tier, priceRange, descFn) {
    for (let lot = 0; lot < LOTS_PER_ITEM; lot++) {
      const pr = priceRange;
      listings.push({
        _uid:     generateUid(),
        category,
        ...
        tier,
        desc:     descFn(item),
        price:    randInt(Math.round(pr.min * (0.9 + lot * 0.1)), Math.round(pr.max * (0.9 + lot * 0.1))),
      });
    }
  }

  // ── ОРУЖИЕ — все пушки каждого активного тира ──────────────
  for (const weapon of Object.values(WEAPONS).filter(w => !w._inactive && activeTiers.includes(w.tier))) {
    const pr = BLACK_MARKET_PRICES.weapon[weapon.tier] || { min: weapon.price, max: weapon.price * 2 };
    addLots('weapon', weapon, weapon.tier, pr, w => `${w.type} · ${w.magSize} патр.`);
  }

  // ── ПАТРОНЫ — все типы каждого активного тира ───────────────
  for (const ammo of Object.values(AMMO).filter(a => !a._inactive && activeTiers.includes(a.tier))) {
    const pr = BLACK_MARKET_PRICES.ammo[ammo.tier] || { min: ammo.price || 5000, max: (ammo.price || 5000) * 2 };
    // Патроны продаём стаками — копируем имя с указанием количества
    const ammoEntry = { ...ammo, name: `${ammo.name} ×${AMMO_STACK_SIZE}` };
    addLots('ammo', ammoEntry, ammo.tier, pr, a => `${a.dmg} урон · ${a.pen} ПРБ`);
  }

  // ── БРОНЯ: шлемы и бронежилеты ──────────────────────────────
  for (const equip of Object.values(EQUIPMENT).filter(e => !e._inactive && activeTiers.includes(e.tier) && (e.slot === 'helmet' || e.slot === 'vest'))) {
    const pr = BLACK_MARKET_PRICES.armor[equip.tier] || { min: equip.price, max: equip.price * 2 };
    addLots('armor', equip, equip.tier, pr, e => `+${e.protection} брони · ${e.weight} кг`);
  }

  // ── СНАРЯГА: рюкзаки и разгрузки ────────────────────────────
  for (const equip of Object.values(EQUIPMENT).filter(e => !e._inactive && activeTiers.includes(e.tier) && (e.slot === 'backpack' || e.slot === 'rig'))) {
    const pr = BLACK_MARKET_PRICES.gear[equip.tier] || { min: equip.price, max: equip.price * 2 };
    addLots('gear', equip, equip.tier, pr, e =>
      e.carryWeight ? `${e.carryWeight} кг ёмкость` : `${e.magSlots || 0} маг · ${e.medSlots || 0} апт.`
    );
  }

  // ── МОДУЛИ — все типы каждого активного тира ────────────────
  const modTypes = ['magazine', 'scope', 'grip', 'stock'];
  for (const mod of Object.values(MODULES).filter(m => !m._inactive && activeTiers.includes(m.tier) && modTypes.includes(m.type))) {
    const prMap = BLACK_MARKET_PRICES.module[mod.type];
    const pr = prMap ? (prMap[mod.tier] || { min: mod.price, max: mod.price * 2 }) : { min: mod.price, max: mod.price * 2 };
    const modWithType = { ...mod, category: 'module', modType: mod.type };
    for (let lot = 0; lot < LOTS_PER_ITEM; lot++) {
      listings.push({
        _uid:     generateUid(),
        category: 'module',
        ...
        icon:     mod.icon,
        tier:     mod.tier,
        desc:     mod.magBonus ? `+${mod.magBonus} патронов` : `+${mod.accuracyBonus || 0} к точности`,
        price:    randInt(Math.round(pr.min * (0.9 + lot * 0.1)), Math.round(pr.max * (0.9 + lot * 0.1))),
      });
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Автосохранение каждые 30 секунд (без симуляции ботов)
  setInterval(() => {
    saveData();
  }, 30000);
