...
function renderCellActions(cell) {
  ...
  if (!cell.looted) {
    ...
    if (cell.type === 'red_paid_safe') {
      const keyData = cell.keyId ? ZONE_KEYS[cell.keyId] : null;
      const hasKey  = keyData && gameData.stash.items.some(i => i.id === cell.keyId && (i.uses || 0) > 0);
      const keyName = keyData ? keyData.name : 'ключ';
      actions.push(`
...
function searchCell() {
  ...
  // Дроп снаряжения / оружия / патронов
...
function openPaidSafe() {
  ...
  const keyIdx = gameData.stash.items.findIndex(i => i.id === cell.keyId && (i.uses || 0) > 0);
  if (keyIdx === -1) {
    showToast('Нет подходящего ключа', 'warning');
    return;
  }

  gameData.stash.items[keyIdx].uses -= 1;
  if (gameData.stash.items[keyIdx].uses <= 0) {
