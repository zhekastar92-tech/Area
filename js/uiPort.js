const UIPort = {
  ...
  renderHeader(){
    ...
    document.getElementById('cur-xp').textContent = p.freeXp.toLocaleString('ru-RU');
  ...
  showShipDetail(s){
    ...
    if(owned){
      ...
      const missing = Math.max(0, s.xpCost - have);
      const canTopUp = pred && missing>0 && profile.freeXp>0;
      actionHtml = `
        ...
        ${canTopUp ? `<button class="btn btn-outline" id="topup-xp-btn" style="margin-top:8px;">Докинуть Free XP (доступно ${profile.freeXp.toLocaleString('ru-RU')})</button>` : ''}`;
    }
    ...
    if(owned){
      ...
      const topupBtn = document.getElementById('topup-xp-btn');
      if(topupBtn) topupBtn.onclick = ()=>{
        const p = Storage.getProfile();
        const pred = Storage.getPredecessor(s);
        const have = p.shipXp[pred.id] || 0;
        const missing = Math.max(0, s.xpCost - have);
        const spend = Math.min(p.freeXp, missing);
        if(spend<=0) return;
        p.freeXp -= spend;
        p.shipXp[pred.id] = have + spend;
        Storage.saveProfile(p);
        this.renderHeader(); this.showShipDetail(s);
      };
    }
  },
