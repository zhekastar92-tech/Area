...
const BotAI = {
  decide(ship, allShips, engine){
    ...
    // приоритет цели: близость + класс + фокус союзников (с антиоверкиллом у статиста) + инстинкт ПЛО у ЭМ + липкость по skill-tier
    const candidates = targetPool.map(e=>{
      ...
      const alliesOnTarget = allies.filter(a=>a.plannedOrder && a.plannedOrder.targetId===e.id);
      if(ship.skillTier==='statist' && alliesOnTarget.length>0){
        // статист прикидывает суммарный ожидаемый урон союзников уже нацеленных на эту цель;
        // если её и так добьют без него — штраф, ищем цель, которая реально нуждается в его орудиях
        const predictedAllyDamage = alliesOnTarget.reduce((sum,a)=>{
          const w = a.def.gk || a.def.torp;
          return sum + (w ? w.damage : 0);
        }, 0);
        score += predictedAllyDamage >= e.hp*1.3 ? -60 : alliesOnTarget.length*15;
      } else {
        score += alliesOnTarget.length*15; // новичок/средний — как раньше, слепой фокус на популярную цель
      }
    ...
    // --- Эндшпиль (после 30-го хода): командное преимущество смещает приоритет к точкам А/Б ---
    if(engine && engine.turn>=30){
      const enemyTeam = ship.team==='player' ? 'enemy' : 'player';
      const myAlive = allShips.filter(s=>s.alive && s.team===ship.team).length;
      const enemyAlive = allShips.filter(s=>s.alive && s.team===enemyTeam).length;
      const advantage = (myAlive-enemyAlive) + (engine.points[ship.team]-engine.points[enemyTeam])/50;

      const pts = [{id:'POINT_A', pos:POINT_A}, {id:'POINT_B', pos:POINT_B}];
      const nearestPt = pts.reduce((a,b)=> dist(ship.pos,b.pos)<dist(ship.pos,a.pos)?b:a);
      const distToPt = dist(ship.pos, nearestPt.pos);

      if(advantage>0.5 && distToPt>2 && Math.random() > weights.noise*0.5){
        // побеждаем — не тянуть бой погоней за последними, а закрывать его через точки
        return { action:'approach', targetId: nearestPt.id };
      }
      if(advantage<-0.5 && distToPt>2){
        const enemyThreateningPoint = allShips.some(e=> e.alive && e.team===enemyTeam && pts.some(p=>dist(e.pos,p.pos)<=8));
        if(enemyThreateningPoint && Math.random() > weights.noise*0.5){
          // проигрываем, но враг реально идёт на очки — надо перехватывать, а не просто убегать
          return { action:'approach', targetId: nearestPt.id };
        }
      }
    }
