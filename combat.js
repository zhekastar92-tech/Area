...
// Броня накапливает урон в рамках раунда — пули бьют последовательно.
// Как только броня обнуляется — следующие пули бьют по голому HP.
// ============================================================
...
function resolveAttack(opts) {
  ...
  var log           = [];
  var totalDamage   = 0;
  var totalArmorDmg = { head: 0, body: 0 };
  ...
  if (isPlayer) {
    ...
    if (actual <= 0) {
      ...
      return { totalDamage: 0, armorDamage: totalArmorDmg, log: log, shots: 0, hits: 0, hitHead: 0, hitBody: 0, misses: 0, armorBlocked: 0 };
    }
    shots = actual;
  ...
  // Точность
  var acc = attackerStats.accuracy || 13;
  if (isPlayer) {
  ...
  // Рабочие копии брони для этого раунда (накопительный урон)
  var workArmor = {
    head: targetArmor.head,
    body: targetArmor.body,
  };
  ...
  for (var i = 0; i < shots; i++) {
    var sr = resolveSingleShot(acc, attackerStats, workArmor);
    totalDamage        += sr.damage;
    totalArmorDmg.head += sr.armorHit.head;
    totalArmorDmg.body += sr.armorHit.body;

    // Применяем урон по броне к рабочей копии (накопительно)
    workArmor.head = Math.max(0, workArmor.head - sr.armorHit.head);
    workArmor.body = Math.max(0, workArmor.body - sr.armorHit.body);
  ...
  return { totalDamage: totalDamage, armorDamage: totalArmorDmg, log: log,
...
// workArmor — текущее состояние брони (уже с учётом предыдущих пуль раунда)
// Коэффициенты:
//   Броня цела (pen < armor): урон по броне = pen*1, сквозной урон = dmg*0.5 - max(0, armor-pen)
//   Броня пробита (pen >= armor): полный урон проходит, броня получает armor единиц урона
//   Без брони: урон = dmg + pen*0.5
// ============================================================

function resolveSingleShot(accuracy, attackerStats, workArmor) {
  ...
  // Попадание: один бросок, зона определяется весами HIT_ZONES
  ...
  var r = Math.random();
  var zone;
  if (r < HIT_ZONES.head.weight) {
    zone = 'head';
  } else if (r < HIT_ZONES.head.weight + HIT_ZONES.chest.weight) {
    zone = 'chest';
  } else if (r < HIT_ZONES.head.weight + HIT_ZONES.chest.weight + HIT_ZONES.arms.weight) {
    zone = 'arms';
  } else {
    zone = 'legs';
  }
  // Для расчёта брони: голова → head armor, всё остальное → body armor
  var armorZone = (zone === 'head') ? 'head' : 'body';
  ...
  // Текущая броня зоны (уже с учётом предыдущих пуль)
  var currentArmor = workArmor[armorZone] || 0;
  var finalDmg     = 0;
  var armorAbsorbed = 0;
  var armorDmgDealt = 0;

  if (currentArmor > 0) {
    // Броня принимает: pen×1 + dmg×0.5
    // HP принимает:    pen×0.5 + dmg×1  (только то что прошло сквозь броню)
    var armorIncoming = pen * 1.0 + dmg * 0.5;

    if (armorIncoming >= currentArmor) {
      // Броня пробита (или добита) этой пулей
      // Урон по HP = pen×0.5 + dmg×1, за вычетом остатка брони сверх пробития
      armorDmgDealt = currentArmor; // снимаем весь остаток брони
      var overflow  = armorIncoming - currentArmor; // сколько "лишнего" пробития
      finalDmg      = pen * 0.5 + dmg * 1.0;
      // Если броня лишь частично пробита — часть урона поглощена
      var absorbed  = Math.max(0, (pen * 0.5 + dmg) * (currentArmor / armorIncoming) * 0.5);
      finalDmg      = Math.max(0, finalDmg - absorbed);
      armorAbsorbed = absorbed;
    } else {
      // Броня не пробита: весь armorIncoming уходит в броню
      armorDmgDealt = armorIncoming;
      // По HP проходит только избыток пробития над бронёй (которого нет) → минимальный сквозной
      // Правило: pen×0.5 + dmg — но глушится бронёй пропорционально
      var ratio     = Math.max(0, 1 - currentArmor / armorIncoming);
      finalDmg      = (pen * 0.5 + dmg) * ratio;
      armorAbsorbed = pen * 0.5 + dmg - finalDmg;
    }
  } else {
    // Брони нет: по мясу полная формула pen×0.5 + dmg×1
    finalDmg = dmg * 1.0 + pen * 0.5;
  }

  finalDmg = parseFloat(finalDmg.toFixed(2));

  if (armorZone === 'head') armorHit.head = armorDmgDealt;
  else                      armorHit.body = armorDmgDealt;
