const UIPort = {
  ...
  renderProfile(){
    ...
    const combined = [...board, {id:'player', elo:p.elo, isPlayer:true}].sort((a,b)=>b.elo-a.elo);
    const place = combined.findIndex(e=>e.isPlayer)+1;
    ...
    document.getElementById('rank-banner').innerHTML = `
      ...
      <div style="color:var(--p-text-sub); margin-top:6px; font-family:Consolas,monospace;">ELO: ${p.elo} · Место в ладдере: #${place}</div>
    `;
  ...
  renderRanked(){
    const p = Storage.getProfile();
    const board = Storage.getLeaderboard();
    const combined = [...board, {id:'player', name:'Вы', elo:p.elo, isPlayer:true}].sort((a,b)=>b.elo-a.elo);
